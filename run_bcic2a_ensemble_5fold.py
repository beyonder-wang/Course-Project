...
def build_model(cfg):
    ...
    if cfg["model"] == "ATCNet":
        atc_cfg = cfg.get("atcnet_kwargs", {})
        kwargs["F1"] = atc_cfg.get("F1", 16)
        kwargs["d_model"] = atc_cfg.get("d_model", 32)
        kwargs["n_windows"] = atc_cfg.get("n_windows", 5)
        kwargs["dropout_conv"] = atc_cfg.get("dropout_conv", 0.3)
        kwargs["dropout_attn"] = atc_cfg.get("dropout_attn", 0.5)
        kwargs["dropout_tcn"] = atc_cfg.get("dropout_tcn", 0.3)
        kwargs["tcn_depth"] = atc_cfg.get("tcn_depth", 2)
        kwargs["attn_drop"] = atc_cfg.get("attn_drop", 0.0)
        kwargs["residual_drop"] = atc_cfg.get("residual_drop", 0.0)
        kwargs["drop_path_prob"] = atc_cfg.get("drop_path_prob", 0.0)
