...
def _train_fold(args, fold, run_dir, channels, num_classes, time_points):
    ...
    # If top_k > 1: save individual predictions and compute ensemble
    if args.top_k > 1 and hasattr(trainer, "top_k_states") and trainer.top_k_states:
        from collections import Counter

        ensemble_dir = os.path.join(run_dir, "top_k_ensemble")
        os.makedirs(ensemble_dir, exist_ok=True)

        all_preds = []
        for rank_idx, (acc, ep, sd) in enumerate(trainer.top_k_states):
            labels = trainer.predict_from_state(sd)
            all_preds.append(labels)
            # Save individual predictions
            fname = f"pred_epoch{ep}_acc{acc:.4f}.txt"
        ...
        ensemble_path = os.path.join(ensemble_dir, "ensemble_majority_vote.txt")
        with open(ensemble_path, "w") as f:
            for lbl in ensemble_labels:
                f.write(f"{int(lbl)}\n")

        # Compare agreement
        best_labels = all_preds[0]  # top-1 state
        n_diff = sum(1 for a, b in zip(best_labels, ensemble_labels) if a != b)
        print(f"\n{'=' * 40}")
        print(f"Top-K Ensemble Analysis (top-{args.top_k}):")
        for rank_idx, (acc, ep, _) in enumerate(trainer.top_k_states):
            print(f"  Rank {rank_idx+1}: epoch {ep}, val_acc={acc:.4f}")
        print(f"  Best single vs ensemble disagree on {n_diff}/{n_test} samples ({100*n_diff/n_test:.2f}%)")
...
def main():
    ...
    parser.add_argument("--top_k", type=int, default=1,
                        help="Number of top validation epochs to keep for ensemble analysis")
