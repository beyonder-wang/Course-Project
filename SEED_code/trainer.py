...
class Trainer:
    def __init__(self, model, train_loader, val_loader, test_loader, lr, epochs,
        ...
        self.best_epoch = 0
        # top-k: min-heap of (neg_acc, epoch, state_dict)
        self._top_k_heap = []
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            # Track top-K model states
            if len(self._top_k_heap) < self.top_k:
                heapq.heappush(self._top_k_heap, (epoch_val_acc, epoch + 1, copy.deepcopy(self.model.state_dict())))
            elif epoch_val_acc > self._top_k_heap[0][0]:
                heapq.heapreplace(self._top_k_heap, (epoch_val_acc, epoch + 1, copy.deepcopy(self.model.state_dict())))
            else:
                no_improve += 1
        ...
        # Sort top-K from best to worst
        top_k_sorted = sorted(self._top_k_heap, key=lambda x: -x[0])
        self.top_k_states = [(acc, ep, sd) for acc, ep, sd in top_k_sorted]
        ...
        return {
            ...
            "top_k_states": [(acc, ep) for acc, ep, _ in self.top_k_states],
        }
    ...
    def predict_from_state(self, state_dict):
        """Run test inference with a loaded state dict and return label list."""
        ...
        with torch.no_grad():
            for test_data in self.test_loader:
                ...
                with self._autocast_context():
                    ...
                    test_logits, _, _ = self._unpack_model_output(test_output)
                test_pred = torch.argmax(test_logits, dim=1)
                all_labels.extend(test_pred.cpu().tolist())
        return all_labels
